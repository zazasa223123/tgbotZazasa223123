...
def _get_name_from_params(params: RepostBotTelegramParams) -> str:
    msg = params.effective_message if params else None
    if not msg:
        return params.sender_name or "пользователь"

    # Обработка пересланных сообщений для python-telegram-bot v20+
    if hasattr(msg, 'forward_origin') and msg.forward_origin:
        origin = msg.forward_origin
        if hasattr(origin, 'chat') and origin.chat and origin.chat.type == Chat.CHANNEL:
    ...
    # Старый синтаксис (сохраняем для обратной совместимости)
    forward_from_chat = getattr(msg, 'forward_from_chat', None)
    if forward_from_chat is not None and forward_from_chat.type == Chat.CHANNEL:
        return forward_from_chat.title or "канал"

    user_id = params.sender_id
    if is_post_from_channel(user_id) and forward_from_chat:
        return forward_from_chat.title or "канал"
...
class _SingularCalloutStyleStrategy(RepostCalloutStrategy):
    ...
    async def callout(self,
        ...
        await params.effective_message.reply_text(response_with_num_and_name,)
